...
export function Nav({ locale, t }: { locale: Locale; t: Dictionary }) {
  ...
  return (
    <header className="sticky top-0 z-50 border-b border-editorial-rule bg-editorial-paper/95 backdrop-blur">
      <div className="relative mx-auto flex h-16 max-w-7xl items-center justify-between gap-1 px-2 sm:gap-4 sm:px-6">
        <Link href={base} className="flex min-w-0 flex-shrink items-center gap-1.5 sm:flex-shrink-0 sm:gap-3">
          <Image src="/logo.svg" alt="" width={40} height={21} priority className="h-7 w-auto flex-shrink-0 sm:h-8" />
          <div className="flex flex-col leading-tight">
            <span className="whitespace-nowrap font-editorial text-lg font-semibold text-editorial-ink sm:text-xl">{t.siteName}</span>
            <span className="-mt-0.5 whitespace-nowrap text-[0.65rem] text-slate-500 sm:text-xs">{t.siteCountry}</span>
          </div>
        </Link>
